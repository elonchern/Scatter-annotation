...
if __name__ == '__main__':
    ...
    for  batch_idx, batch in enumerate(tqdm(dataset_loader)):
        ...
        counts = np.bincount(scatter_kitti.flatten().astype(int), minlength=20)
        gt_counts = np.bincount(gt_label,minlength=20)
        total_counts = total_counts + counts
        total_gt_counts = total_gt_counts + gt_counts

        
    np.save('kitti_total_counts_9.npy', total_counts)
    np.save('kitti_total_gt_counts_9.npy',total_gt_counts)     
    print(total_counts) 
    print(total_gt_counts)   
